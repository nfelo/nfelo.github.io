...
4. **Replace the all-or-nothing forecast gate.** If preserving the network's top pick is a product requirement, move towards the score forecast only as far as that constraint permits. This keeps exactly the same top pick while improving every proper score in the replay.
